const menusservice = require("../service/menuService");
...
methods.CreateUser = async function (req, res) {
    try {
        var userData = await menusservice.CreateUser(req.body);
        if (!userData) {
...
methods.GetUser = async function (req, res) {
    try {
        var userData = await menusservice.GetUser();
        if (!userData) {
...
methods.GetUserById = async function (req, res) {
    try {
        ...
        var userData = await menusservice.GetUserById(req.params.id);
        if (!userData) {
            return handler(res, mc.ERROR_MSG, mc.ERROR_CODE, 'User Is Not Get', null);
        }
        else {
...
methods.DeleteUser = async function (req, res) {
    try {
        ...
        var userData = await menusservice.DeleteUser(req.body, req.params.id);
        if (!userData) {
...
methods.UpdateUser = async function (req, res) {
    try {
        console.log("Delete User")
        console.log(req.params.id, req.body);
        var userData = await menusservice.UpdateUser(req.body, req.params.id);
        if (!userData) {
            return handler(res, mc.ERROR_MSG, mc.ERROR_CODE, 'User Is Not Delete', null);
        }
        else {
